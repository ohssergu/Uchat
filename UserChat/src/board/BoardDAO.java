...
public class BoardDAO {
 ...
 DataSource dataSource;
	 ...
	 public ArrayList<BoardDTO> getImageList(String pageNumber) {
		 ArrayList<BoardDTO> boardList = null;
		 Connection conn= null;
		 PreparedStatement pstmt = null;
		 ResultSet rs = null;
		 String SQL = "SELECT * FROM BOARD WHERE boardGroup > (SELECT MAX(boardGroup) FROM BOARD) - ? AND (boardGroup <= (SELECT MAX(boardGroup) FROM BOARD) - ? and boardfile IS NOT NULL AND (boardfile LIKE '%jpg%' OR boardfile LIKE '%png%' OR boardfile LIKE '%gif%' OR boardfile LIKE '%jpeg%')) ORDER BY boardGroup  DESC, boardSequence ASC";
		 try {
			 conn = dataSource.getConnection();
			 pstmt = conn.prepareStatement(SQL);
			 pstmt.setInt(1,  Integer.parseInt(pageNumber) * 10);
